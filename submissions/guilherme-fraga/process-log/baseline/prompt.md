# Baseline: brief colado sem nenhum contexto

Rodado com `claude -p` (modelo padrão da CLI, sessão limpa, sem acesso aos CSVs, sem instruções minhas além do brief). Objetivo: saber o que a IA entrega sozinha, pra medir o que a nossa solução faz a mais.

## Prompt exato

````text
Resolva o desafio abaixo. Entregue a solução completa (código e explicação da lógica de scoring).
...
- Claude Code CLI 2.1.273, `claude -p --no-session-persistence --tools ""`, modelo padrão da conta (Opus 5), effort padrão da conta (xhigh).
- Diretório vazio, sem os CSVs, sem memória, sem instruções minhas além da linha "Resolva o desafio abaixo".
- Primeira tentativa, sem nenhuma instrução extra: o modelo tentou chamar ferramentas (`ls`, `find sales_pipeline.csv`) mesmo sem ter nenhuma, e a saída veio como XML de chamada de ferramenta. Descartada.
- Segunda tentativa, com uma linha a mais de system prompt: *"Você não tem acesso a arquivos, internet nem ferramentas. Responda somente em texto, com o código completo inline."* É a que está em `output.md`, sem nenhuma edição.
- Tempo de resposta: cerca de 20 minutos.
